...
def test_summary_after_upload():
    client.post(
        "/upload",
        files={"file": ("bio.txt", b"Photosynthesis converts light energy into chemical energy in plants.", "text/plain")},
        headers={"X-User-Id": "dana"},
    )
    r = client.post("/summary", json={}, headers={"X-User-Id": "dana"})
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_flashcards_after_upload():
    client.post(
        "/upload",
        files={"file": ("chem.txt", b"Water is H2O. It boils at 100 degrees Celsius at sea level.", "text/plain")},
        headers={"X-User-Id": "eve"},
    )
    r = client.post("/flashcards", json={"count": 3}, headers={"X-User-Id": "eve"})
    assert r.status_code == 200, r.text
    body = r.json()
    assert len(body["flashcards"]) >= 1
    assert "front" in body["flashcards"][0]
    assert "back" in body["flashcards"][0]


def test_quiz_after_upload():
    client.post(
        "/upload",
        files={"file": ("hist.txt", b"The French Revolution began in 1789.", "text/plain")},
        headers={"X-User-Id": "frank"},
    )
    r = client.post("/quiz", json={"count": 2}, headers={"X-User-Id": "frank"})
    assert r.status_code == 200, r.text
    body = r.json()
...
def test_study_tools_empty_without_upload():
    r = client.post("/summary", json={}, headers={"X-User-Id": "ghost-user"})
    assert r.status_code == 200
    assert "upload" in r.json()["summary"].lower()
