...
# In serverless this gets re-initialized per cold start; that's fine.
...
def _reindex_from_disk():
    """Re-ingest all previously uploaded files from local storage into the vector store on startup.

    This is only needed for VECTOR_BACKEND=local (in-memory) since it loses data on restart.
    With VECTOR_BACKEND=bedrock_kb, documents persist in Bedrock Knowledge Base.
    """
    if config.vector_backend != "local" or config.storage_backend != "local":
        return
    try:
        all_keys = storage.list()
    except Exception as e:
        logger.warning(f"[reindex] Could not list files: {e}")
        return
    ...
    for key in all_keys:
        ...
        if len(parts) < 3:
            continue
        user_id, doc_id, filename = parts[0], parts[1], "/".join(parts[2:])
        try:
            data = storage.get(key)
            text = handlers._extract_text(filename, data)
            if text.strip():
                vector_store.ingest(
                    doc_id=doc_id,
                    text=text,
                ...
                reindexed += 1
                logger.info(f"[reindex] Loaded '{filename}' (doc_id={doc_id[:8]}…)")
        except Exception as e:
            logger.warning(f"[reindex] Skipped {key}: {e}")

    if reindexed:
        logger.info(f"[reindex] ✅ Re-indexed {reindexed} document(s) from disk.")
    else:
        logger.info("[reindex] No previously uploaded files found to re-index.")
...
app = FastAPI(title="StudyBot — W7 Capstone Starter", lifespan=lifespan)
...
@app.post("/summary")
def summary(req: StudyToolRequest, x_user_id: str | None = Header(default=None)) -> dict:
    """One-page study guide + five key concepts from uploaded slides/PDF."""
...
@app.post("/flashcards")
def flashcards(req: StudyToolRequest, x_user_id: str | None = Header(default=None)) -> dict:
    """Generate flashcards (front/back) from uploaded lecture content."""
...
@app.post("/quiz")
def quiz(req: StudyToolRequest, x_user_id: str | None = Header(default=None)) -> dict:
    """Multiple-choice quiz from uploaded lecture content."""
